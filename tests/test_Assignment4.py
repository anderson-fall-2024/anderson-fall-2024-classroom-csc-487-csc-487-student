...
def test_1():    
    ...
    answer_thetas = py487.functional.minimize_gradient_descent([gradients_w1,gradients_w2,gradients_b],0.1,[0.5,-0.2,2.5])
...
def test_2():    
    ...
    answer_thetas = py487.functional.minimize_gradient_descent_analytically(R_func,0.1,[0.5,-0.2,2.5],0.01)
    assert np.all(np.abs(np.array(solution_thetas)-np.array(answer_thetas)) <= 0.0001)